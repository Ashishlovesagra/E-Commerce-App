  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await axios.post(
        "http://localhost:1009/api/v1/category/create-category",
        { name }
      );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const data = await axios.put(
        `http://localhost:1009/api/v1/category/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data.data.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const data = await axios.delete(
        `http://localhost:1009/api/v1/category/delete-category/${pId}`
      );
      if (data.data.success) {
