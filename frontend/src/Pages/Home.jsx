...
function Home() {
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get(
        "http://localhost:1009/api/v1/products/product-count"
      );
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(
        `http://localhost:1009/api/v1/products/product-list/${page}`
      );
      setLoading(false);
  ...
  const filterProducts = async () => {
    try {
      const { data } = await axios.post(
        "http://localhost:1009/api/v1/products/product-filters",
        { checked, radio }
      );
      setProduct(data?.products);
    } catch (error) {
    ...
    <>
        ...
        <div className="col-md-9">
            ...
            {product.map((prod) => (
              <div
                ...
                key={prod._id}
              >
                <img className="ProductImage" onClick={() => navigate(`/product/${prod.slug}`)} src={prod.image} alt="" />
     ...
     <div className="container box">
      <div className="containerH1">
        ...
        <div className="h1-paraBox">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor minim veniam, quis nostrud reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur</div>
      </div>
      <div className="containerH1">
      <div className="h1-imageBox">
        <img src={Refund} alt="" />
