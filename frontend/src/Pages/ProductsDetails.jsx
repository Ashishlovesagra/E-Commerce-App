...
const ProductsDetails = () => {
    ...
    const getProduct = async () => {
        try {
            const {data} = await axios.get(`http://localhost:1009/api/v1/products/get-product/${params.slug}`)
    ...
    const getSimilarProducts = async (pid,cid) => {
        try {
            const {data} = await axios.get(`http://localhost:1009/api/v1/products/related-product/${pid}/${cid}`)
    ...
    <>
      <div className="row">
        <div className="col-md-5">
            <img src={product.image} style={{width:"80%",marginLeft:"10%"}} alt="Product Image" />
        </div>
        <div className="col-md-5">
            <h1>{product.name}</h1>
            ...
            <div className="row">
            <p>Offer Price: <span className="productPriceTitle">  ₹ {product.price}/-</span></p>  
            </div>
            <div className="row">
            <p>Original Price: <span className="productOPriceTitle">  ₹ {product.oPrice}/-</span></p>  
            </div>
            <div className="row">
            <p>Quantity: <span className="productRatingTitle"> {product.quantity}</span></p>  
            </div>
      ...
      {relatedProduct.map((prod) => (
              <div className="ProductCart" key={prod._id} onClick={() => navigate(`/product/${prod.slug}`)}>
